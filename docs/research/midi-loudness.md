...
and the songs that need answering are the quiet ones.

A measurement rendered through the bank that
will play the song closes the spread entirely. A measurement rendered through some *other* bank closes
it to between 3.6 and 9.8 LU, depending on which two. Across six banks, that is no better on average
than reading a level out of the events with no synthesizer at all. That estimate closes it to 5.0 LU,
costs 0.9 ms a song and needs no instrument bank. It also clips **less** than the machine does today,
because the rule that raises a quiet song lowers a loud one. The staged packaging tools carry no bank, so the
...
loud. That is the difference between a mix and the same mix with a lead line over it.
...
over a measured gap of 4.8 dB against a video and 16.4 dB against a commercially mastered MP3. This is
12.8 dB between one MIDI song and the next, on the same machine at the same amplifier setting.

**The `spread` column ranks banks; it does not say what a room hears.** [inferred] Seven songs
chosen to compare banks give a number a hundredth the size of the corpus figure. Both are correct
for what they measure. A bank's own contribution to the spread is real and small beside the
files'.
...
and so the difference between songs. The corpus arrives compressed: the fit from Colombo to Aspirin
has a slope of 0.657. A bank that compresses cannot report how far apart two songs are.
...
power, and the powers of simultaneous notes add. The estimate gates the result the way R128 gates a
real signal.
...
Two of the three floor above zero. Each floors at its own error, which is about 2.56 times the
...
**The middle column is the best pair of six, not a typical one.** Read against section 2's matrix,
it runs from 3.6 LU for that pair to **9.8 LU** for Aspirin against MuseScore. The median pair gives
...
**Every one of them beats doing nothing, and only the last is clearly worth a bank.** [inferred] A design
therefore does not choose between "render or estimate". It asks whether the bank doing the measuring
is the bank that will play the song. Where it is, rendering is unbeatable. Where it is not,
...
is files that sound right and play too low. Attenuation-only closes the spread by bringing everything
down to the quiet ones, which is the opposite.
...
is sparse or lightly played, not because it is compressed. So its peak falls with its loudness, and
the room to raise it comes free. Nothing in the corpus behaves like a mastered recording, which is
...
That is too loose to bound a gain against full scale.

**So a boost taken from an estimate is capped rather than bounded.** Section 5 says what that
...
bank that will play the song licenses a properly bounded boost. Section 2 reaches the same conclusion
about the level.
...
is testable against the same 989 rows. Take the residual against the rendered figure. Then correlate
it with what each song is made of, as shares of the power the estimate itself counts.
...
[inferred] What is left is the difference between one preset and another inside a family. That is a
...
The estimate read it as 1.0 dB **loud**, and the machine attenuated it. That is 3.4 LU in the wrong
direction, on a file somebody had already picked out by ear. The eighteen-constant correction would have left
...
window reads 0.40 LU quiet with a per-song sd of 1.12 LU. The worst case is 7.7 LU, and **12% of
songs differ by more than 1 LU**. A packaging measurement would have to render the whole thing, at 2.76 s a
song.
...
`tools/dist/cmd.sh` stages each tool as its executable and a README. It says in place that there is
no asset-fetching step, because none of them has a SoundFont. So rendering at packaging time needs an
answer to where the bank comes from. It could ship 31 MiB beside `km-pack`, or fetch 262 MiB for the
recommended one. Or the user points at an `.sf2` they happen to have.
...
a bank buys the accuracy of the free answer. It pays for that with a render, a bank, and `km-audio`
in the packager. What it does buy is the case where the searched bank is the playing bank. That case
is worth having: it is the owner packaging for one machine they control.
...
outside those 15 gives the machine a level it cannot convert to an offset. The existing fallback of
−22.0 is 9.6 dB wrong for `somsak` at −12.4, which is worse than no levelling. One answer restricts
measuring to banks that carry a reference. The other has the packager store the song's offset from the
package's own mean, and the bank drops out of the record.

[inferred] The second costs accuracy on a small package. The standard error of a package mean is
5.11/√n, so 0.72 LU over 50 songs and 0.23 LU over 500. And a package curated to one mood would carry
a mean that is the mood rather than the bank.
...
**The two kinds are the same loudness as each other**, within 1 dB of median. Both sit about
**8 dB** above the −22.8 a MIDI song is now brought to on the recommended bank. So the gap a room
hears between a MIDI song and a media one is that 8 dB. It closes entirely once the media carries
numbers.

**Nothing reached `MIN_GAIN`.** The −20 dB floor exists to stop a wrong measurement muting a song.
The hottest file in 286 is −5.7 LUFS, which asks for −15.3 dB. The floor has room to spare
...
`somsak` is worse and is not a leader. Over 387 songs its mean true peak is **+1.15 dBTP**, and its
loudest is **+25.1**. Eleven of those songs metered at or above full scale. That run excluded them as
divergences rather than as loud songs. Its `volume` of 0.22 is what stands between it and
...
2. **Whether the target is the bank's own mean.** If it is, all three kinds of song sit at the figure
   media is already brought to. The media rule then needs no change at all. Any target below that mean
...
4. **Estimate, render, or both.** Rendering through the playing bank leaves nothing; rendering through
   an arbitrary one lands where the free estimate lands. The numbers point at a design that renders when
   the bank is named and estimates otherwise. It stores two things in one field.
