...
| [`meta-quest.md`](meta-quest.md) | The machine on a Meta Quest 3 headset | **Measured on the hardware.** The existing Android APK runs as a flat Horizon OS panel with no change to anything, and every song kind plays. The remote runs beside it as a second panel. An immersive build is out of reach, because SDL's OpenXR support is unreleased. A sideload rather than a carrier. |
