...
- **Its own round trip is what caught that**, unpacking the built package and starting each of the
  ...
  the setup program is macOS-only and is built on a Mac rather than by the release workflow, so that
  check is the only thing standing between a payload rule changing and a release shipping short.
