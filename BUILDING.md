...
**There is no Makefile, and nothing to install to get one.** Everything routine is a `cargo`
command; the longer ones have aliases in the committed `.cargo/config.toml`, so `cargo km-test` is
the same thing as typing the full line and neither is privileged. The shell scripts under `tools/`
are the jobs cargo has no business doing — fetching things from the network, building a Debian
package in Docker, cross-compiling for Android, starting three servers and driving a browser for the
screenshots.

**This table is every alias in `.cargo/config.toml`, and it is meant to stay that way** — a table
that lists two thirds of them sends somebody to `cargo run -p`, which is the spelling that file
exists to replace.
...
its two members need TLS from `reqwest` and `km-package-builder` needs it to have none — so `-p`
cannot see them and anything that wants one has to name `--manifest-path`. That path is written
once in `.cargo/config.toml` now, and `task fmt` forwards to it rather than keeping a copy. The
qualifier goes after the verb, as in `km-lint-no-video`: `km-lint-assets` lints the assets
*workspace*, where `km-admin-lint` would read as linting the one crate of that name.

**`km-lint` and `km-lint-assets` are the two aliases that will not take an extra flag**, because
their trailing `-- -D warnings` swallows whatever follows — `cargo km-lint --locked` hands
`--locked` to clippy as a lint name. That is why `tools/platform/linux/check.sh` spells its own
`--locked` clippy and test runs out in full and then *asserts* that the aliases carry the same
feature list, rather than calling them.
...
what this project is; `km-pack` and `km-pkgbuild` do not, because `km-pack book` and
`km-pack check` never open a song's bytes and a MIDI or MP3+G corpus scans without ffmpeg — so those
two ask for video by name, as `cargo km-pack-video` and `km-pkgbuild-video`.

### `Taskfile.yml`, the one list of everything

There is also a [Taskfile](https://taskfile.dev), and it is not the paragraph above being taken back
— it is a table of contents for it. Every task wraps a `cargo` alias or a script from `tools/`, and
the wrapped thing stays the real one. What `task` adds is that `task --list` prints all of them at
once with a sentence beside each, which is the one thing neither a cargo alias file nor a directory
of scripts can do for you.
...
default and that has not changed. But every command in this file that carries no suffix is the video
build: `cargo km-build`, `cargo km-test`, `cargo km-lint`, `cargo km`, and `task check`. So a machine
you intend to develop on wants one more setup step, once:

```sh
tools/setup/fetch-ffmpeg.sh        # or: task ffmpeg
```

[Video songs](#video-songs) is what that does and where it puts things. If you would rather not —
you are reading a diff, or checking that the video-less build still compiles — the `-no-video` and
`:no-video` twins in the tables above need none of it. See the `Video is the default build` decision
...
The MSVC linker is what Rust's `x86_64-pc-windows-msvc` target uses. If you have Visual Studio or
the **Build Tools for Visual Studio** (the "Desktop development with C++" workload), you already
have it — `cl` does not need to be on `PATH`, only the linker needs to be findable, which the
toolchain arranges.
...
in a global `env:`, the release script `tools/platform/macos/app-bundle.sh` for the build it drives,
and `ci.yml` at the top level. What is left is a `cargo` command typed by hand — which is exactly
where the panic is hardest to place, hence the export above. The boundary, and why an environment
variable is a safe thing for the Taskfile to carry when a feature list is not, is
...
It asks for sudo, or runs directly as root in a container. The list inside it is SDL's own
`README-linux` list rather than a minimal one, deliberately: SDL's configure aborts on the *first*
missing dependency and names only that one, so trimming it costs a build per package to rediscover.
...
tests need both — so something has to be named either way.

**And on Linux `--all-features` cannot work at all**, which is why nothing here uses it. It turns on the package builder's `desktop` feature, whose `wry` links libwebkit2gtk — a library a Linux
build of that tool is deliberately never given, because such a build would not start on a machine
without it. So the one flag meaning "everything" asks Linux for the one thing it will not provide.

The feature list lives once in `tools/setup/features.sh`, is spelled a second time in
`.cargo/config.toml` because a cargo alias cannot source a shell file, and `tools/platform/linux/check.sh`
asserts the two agree. `cargo km-test` carries the video list; `cargo km-test-no-video` carries the
two `testing` features alone. Do not write either out by hand.
...
The app is quiet by default — `info` and nothing per-frame. `-v` adds its own debug stream, `-vv`
adds everybody's, `RUST_LOG` overrides both, and `logging.level` in `settings.json` says the same
thing in `RUST_LOG`'s grammar for a machine nobody types at. `--frame-stats` is separate on purpose: it decides
whether the frame rate is *measured*, which is not a question about how much detail you want in a
log, so no log level turns it on and turning it off costs nothing. `KM_FRAME_STATS=1` does the same
where there is no command line to pass it on.

**A checkout opens no mDNS daemon.** `.cargo/config.toml` sets `KM_NO_MDNS=1` and `Taskfile.yml` sets
it for what it drives, so `cargo km-test`, `cargo run` and `task check` bind no multicast socket and
Windows has nothing to raise a firewall dialog about. `KM_NO_MDNS=0` in front of one command asks for
mDNS back — which is what testing discovery from a phone needs — and the two tasks that launch a
staged build clear it already, because running one of those is how you watch the shipped behaviour.
Nothing an owner installs reads the variable; the reasoning is
[`KM_NO_MDNS` declines the multicast socket, and one function honours it](docs/decisions/api-and-network.md#km_no_mdns-declines-the-multicast-socket-and-one-function-honours-it).
...
[staging script](#releases) builds it — Windows, macOS, Linux and Android — and each stops with a
message rather than quietly staging the lesser build if ffmpeg is missing. `--no-video` is the
...
**in cargo** — and there are now three defaults here, which sound contradictory and are answers to
three different questions.

- **The cargo one is off.** "What must somebody install before a fresh clone compiles at all", and
  ffmpeg is the only C dependency in this project you have to install yourself — so
  `cargo build --workspace` needs neither it nor libclang. Untouched, and it is what keeps a clone
  cheap.
- **The release one is on.** "What does somebody receiving a folder get to play", and there the
  answer is everything a catalog can hold. A build without the feature still catalogs, searches
  and queues video songs, and says at startup that it cannot play them.
- **The command one is on.** "Which name is short" — `cargo km-build`, `cargo km-test`,
...
So everything below is what building one needs, once — and on a machine you intend to develop on,
that is a setup step rather than an optional extra.
...
- **macOS** — **builds** a pinned LGPL ffmpeg from source into that same cache: about a minute,
  once. A build rather than a download because nothing publishes a prebuilt *shared* LGPL macOS
  ffmpeg with headers, and because a release redistributes what it links — see
  [Releases](#releases). `--homebrew` takes Homebrew's GPL build instead. libclang comes from the
  Command Line Tools you already have. **`brew install openh264` first**: the pinned configure asks
  for it, because it is the H.264 encoder a streaming machine produces its picture with and the only
  one an LGPL ffmpeg may carry. The script checks for it and stops before the build rather than
  leaving the answer inside a configure log.
- **Linux** — installs `libav*-dev`, `libopenh264-dev` and `libclang-dev` through apt, dnf or
  pacman. Asks for sudo first and says so.

**The cache is keyed by the ffmpeg version, so a prefix already in it is taken as it stands.** What
the pinned build is *configured* with is not part of that key, and a change to it therefore reaches
a machine that has built before only when the directory is removed or the script is given `--force`.
Nothing reports the difference: the libraries are there, the build links them, and the absence shows
up as a program refusing to start a stream. Windows re-downloads on a changed pin and has none of
this.

After that, **plain cargo builds video** — there is nothing to export and nothing to source:
...
`$CARGO_HOME/config.toml` between a pair of marker comments it rewrites in place. Cargo applies
`[env]` to every build script it runs, so the setting reaches `ffmpeg-sys-next` from any shell, from
a script, and **from an IDE that invokes cargo itself** — none of which inherit an `export` you
typed. A real environment variable still wins, since cargo does not mark these `force`; that is how
a hand-built ffmpeg gets used instead.
...
Packaging video songs shells out to whatever `ffmpeg` is on `PATH`, which is a separate thing from
the libraries above and only needed when a file falls outside the playback profile:
...
copied byte-for-byte when it already matches, which a `yt-dlp` download normally does.

### If your IDE fails with `ffmpeg-sys-next` errors

An IDE that builds the whole workspace (RustRover, VS Code with rust-analyzer set to `--workspace`)
compiles every member. `km-video` is a member, so its ffmpeg dependency is behind **its own**
default-off feature and a bare workspace build compiles it to an empty library — no ffmpeg needed.
If your IDE still fails, it is building with all features on — RustRover's Cargo settings have such a
switch, and rust-analyzer has `cargo.features`. Either turn that off, or run `tools/setup/fetch-ffmpeg.sh`
and let it stand: because it records the paths in cargo's `[env]` rather than in your shell, the IDE
picks them up on its next build with nothing further to configure. That is the practical reason to
prefer the cargo config over exported variables — an IDE launched from a desktop shortcut never saw
your shell.

**The trade to know about.** With the feature off, `km-video` is an empty crate, so your IDE offers
no completion or error checking inside `crates/playback/km-video/src/lib.rs` — the file is `#![cfg]`-ed out
entirely. If you are working *on* the video decoder rather than around it, run
`tools/setup/fetch-ffmpeg.sh` once and then turn the `video` feature on in your IDE; everything else in the
...
The packaging and curation commands — `km-package-builder` and `km-pack` — are how a catalog gets
made, so they are documented for their users in
[`README.md`](README.md#getting-a-corpus-into-shape). From a checkout each has an alias in the table
above — `cargo km-pkgbuild` and `cargo km-pack` — and those are the spelling to use: a
`cargo run -p` typed from memory is the one that comes out wrong.
...
the machine** and nothing about it touches `assets/` or any carrier — see the [`A downloadable song
pack`](docs/decisions/repository.md#a-downloadable-song-pack) decision, which also says why this is
the only pack that could be built at all.
...
It needs the network once — the source hymnal is fetched into the asset cache and verified against a
pinned SHA-256 — and **`abc2midi`**, which is not otherwise a prerequisite of anything here:
...
libraries, no build system — and it picks one by **trying** `$CC`, `cc`, `gcc` and `clang` in turn
...
A system package is preferred where one exists and is found first — `apt install abcmidi`,
...
modern UCRT header refuses that outright. A MinGW gcc has no `_MSC_VER`, never takes that branch,
and builds it in one line — `winget install BrechtSanders.WinLibs.POSIX.UCRT` is one.
...
twenty states that are easy to get wrong, on a flat background, for looking at in a pull request. The
second produces the handful of pictures that are *published*, over a real wallpaper and against a
real catalog — so it needs a folder of songs in `KM_CORPUS`, and without one it deliberately writes
to `target/screenshots` rather than touching what is committed. It keeps the package builder's
recent-folder list under `KM_SHOT_DIR` rather than in yours, for the same reason it gives the other
two servers a `--data-dir` of their own.
...
`-console` twin, which is a diagnostic rather than the one to reach for. Which is which, and why,
is the `The machine's console window` decision in
[`docs/decisions/distribution.md`](docs/decisions/distribution.md); no
...
the two `.ipa` files and nothing else does, so naming the platforms a run carries takes the rest out
of the table, out of the count and out of the body's download table at once. A carrier of a platform
...
`--upload` creates a **draft** and fills it; publishing is
`gh release edit v<version> --draft=false`, and a tag whose release is already published is refused
...
The run substitutes `@VERSION@` and the carol pack's name into
`dist/release/<version>-notes.md` and sends that, so the tracked file names no version. Edit its
`What changed` list before a cut. `--notes-file <path>` overrides it, and a draft's body is
rewritten on every `--upload`, so a corrected sentence is a re-run.

**Linux carries three**, and the third is `karaokemachine-tools`: the package builder, the offline
remote and km-admin, in a package of their own that the machine Recommends. It is a download beside
the machine's `.deb` rather than something `apt` fetches, there being no repository to fetch it from,
so `task dist:deb:tools` stages it and the release page names both files in one `apt install` line.
...
builds sit in the same folder under names ending in `-unnotarized` and `-unsigned`, and neither is
gathered.
...
`dist/setup/windows/karaokemachine-setup-<version>-windows-x86_64.exe` — one Inno Setup installer
holding all seven products behind component checkboxes, so somebody who wants only the offline remote
pays for neither the instrument bank nor the video libraries. It installs **per-user** into
`%LOCALAPPDATA%\Programs` and raises no UAC prompt, offers to put itself on your `PATH` and to open
`.kmbuild` corpus files, and **leaves your songs, settings and catalog alone when uninstalled** —
saying where they are on its way out. It needs Inno Setup 6
(`winget install JRSoftware.InnoSetup`), which it looks for in the per-user location `winget` uses
before the Program Files ones.

The installed folder holds the **windowed** executable of each product and not the console twin —
the `What an installed build contains` decision in
[`docs/decisions/distribution.md`](docs/decisions/distribution.md) says why. There is no
`--no-video` installer,
and a payload with no ffmpeg in it stops the build rather than producing a setup that lists video
songs and cannot read them.
...
exclusion where its own carrier check would otherwise object — the payload-coverage check on
Windows, `excluded()` on macOS — and both round trips now read the installed file.

What SmartScreen and Gatekeeper show the first person to run an unsigned build, and how the macOS
uninstaller is found, are in [`README.md`](README.md#installing) — they are what a recipient sees
rather than what a build does.

**macOS has one too, and it is an Apple installer package.** `tools/platform/macos/installer.sh` builds
`dist/setup/macos/karaokemachine-setup-<version>-macos-<arch>.pkg` — that name when it is notarized, and
`-unnotarized` or `-unsigned` before the extension when it is not — the same seven products behind the
same component ticks, so somebody who wants only the offline remote pays for neither the
instrument bank nor the video libraries. The four applications go to **`/Applications`** and the six
command-line tools to `/usr/local/karaokemachine`, with symlinks in `/usr/local/bin`, which is
already on your `PATH` — so there is no "add me to your `PATH`" tick and nothing edits a
...
registry; on macOS the `.kmbuild` association is declared by a bundle sitting in `/Applications` and
the "`PATH` entry" is a symlink in `/usr/local/bin`, and both of those belong to the machine.
...
[`tools/platform/macos/installer.sh`](tools/platform/macos/installer.sh)'s signing section, so this
takes no setup on the machine the certificates live on. To sign as yourself, either replace them
there — `security find-identity -v` lists what your keychain actually holds — or override without
touching a tracked file, which is what an environment variable is still for:
...
names that `pkgutil --check-signature` prints from any package signed with them, and the profile is
only a keychain label — the Apple ID and app-specific password behind it stay in the keychain, put
there once by `xcrun notarytool store-credentials`. What they do carry is a person's name, which is
why they are argued under *Published identity* in the
...
decision rather than simply pasted in.

Two identities because they are two certificate types — Application signs the bundles, Installer
signs the archive — and setting only the first is refused, because Gatekeeper judges the archive.
Notarizing is a separate flag rather than part of signing: it goes to Apple and takes minutes, and
since Catalina it is the half that actually removes the dialog, a Developer ID signature on its own
not being enough for a downloaded file. **The defaults apply on that flag alone**, so every other
path is still ad-hoc and unsigned — which is what keeps a clone with no Apple account able to build a
release. **Never combine it with `--no-build`**: signing happens while the payload is staged, so
skipping the staging leaves an ad-hoc payload inside a signed wrapper and only the round trip at the
very end notices.

**Android has one variable and the same shape.** `KM_ANDROID_KEYSTORE` names the release key, and
without it the debug key signs the release build, so a fresh clone builds an APK with nothing set up:
...
The four Docker-driven scripts above build their images on demand, so the first of them on a new
machine is slow and the rest are not. `tools/platform/linux/prewarm.sh` does that up front instead, which is
worth running once if you would rather pay it deliberately; `--check` reports what is cold without
building anything. It is never required — every script still builds what it needs if it is missing.
...
and the version, and a cargo feature changes neither — two builds in one directory would overwrite
each other.

**The macOS setup program is named for its signing state on the same rule** — the marker goes on the
declined build, so the notarized package, the only one worth handing anybody, keeps the plain name.
Three names because there are three states and the middle one is not a rounding error: `spctl`
refuses a signed-but-unnotarized archive exactly as it refuses an unsigned one. They coexist rather
than overwriting, so rebuilding ad-hoc for a local test cannot destroy a notarized package;
`clean:old` is unaffected, since it reads the version as the field after the app name and still
finds it in a marked name.
...
**On macOS the `.app` is the windowed form and the bare executable is the console one**, which is the
same pair Windows spells as `km-remote.exe` beside `km-remote-console.exe`. So `dist/bin/macos` holds
...
staging scripts as everything above and copies what they produced, so the two cannot disagree about
what a release contains. `--no-build` skips straight to the copying. The folders carry no version and
`--zip` produces one that does.

`tools/platform/linux/check.sh` runs CI's Linux job — fmt, clippy and the whole test
suite — in the same Debian container, in about a minute against a warm cache. Worth running before a
push from Windows: a `Path` treating `\` as an ordinary character, a missing system library and a
wrong `#[cfg]` are the three failures that only appear off Windows, and all three have happened.

**All three desktops build video, and each pays for it differently — but all three now travel.**
Windows is the easy case: `tools/platform/windows/dist.sh` stages the four ffmpeg DLLs the executable actually
imports beside it, with their LGPL terms, and then proves the folder is self-contained by starting it
with ffmpeg stripped from `PATH` — so the folder copies to a USB stick and runs. **Both Linux carriers
carry the same built ffmpeg**: `tools/platform/linux/deb.sh` puts it in `/opt/karaokemachine/lib` and
the package `Depends` on no `libav` at all, which is what lets an appliance install decline X — see
...
a distribution packager wants. **The tarball had to do this first**, because a folder can name no
dependency and Debian's ffmpeg cannot travel inside one: it
is GPL-2+ by Debian's own copyright file while this workspace is `MIT OR Apache-2.0`, and it drags 93
shared libraries and 97 MiB behind it. `tools/platform/linux/ffmpeg-lgpl.sh` builds the same pinned LGPL
ffmpeg the macOS branch does, inside the build container, and the tarball carries those four at about
13 MiB, linking nothing but libc. **macOS costs the most work**, because
a Mach-O names its libraries by the absolute path it was linked at: `tools/platform/macos/app-bundle.sh` copies
the four libraries *and the nine more they pull in* into `Contents/Frameworks`, rewrites every load
command to `@rpath`, and re-signs each file — `install_name_tool` invalidates the ad-hoc signature
these carry, and dyld refuses to load a library whose signature does not match. It then asserts that
nothing loads by absolute path. `tools/dist/cmd.sh` does the same for the km-pack and
km-package-builder folders, into a `lib/` beside the executable. All three are written up in
[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).

Bundling means **redistributing ffmpeg**, which is why `tools/setup/fetch-ffmpeg.sh` builds a pinned LGPL
ffmpeg from source on macOS rather than taking Homebrew's. Homebrew's is the GPL configuration
(`--enable-gpl`, with x264 and x265), and this application never encodes through those libraries at
all — so linking it meant shipping a GPL build carrying 25 MB of encoders nothing could call. The
source build takes about a minute, once per machine, and produces four dylibs and 15 MB that depend on
nothing outside macOS. `--homebrew` takes Homebrew's instead, and the license note staged beside the
...
[`ports/machine/android/README.md`](ports/machine/android/README.md) is the full account — why there is no
`externalNativeBuild` block anywhere in the project, what is copied from SDL and what is ours, why
`minSdk 26`, and why `JAVA_HOME` must name a JDK 17 or 21 rather than the JBR that ships with
Android Studio.
...
loads `armeabi-v7a` alone, so an APK without it installs on a phone and fails on a television.
...
- **Dependencies are opted into per crate.** The root manifest carries the full version table, but a
  member lists only what it needs — so nothing is compiled until it is used and **each crate's
...
- **One dependency comes from git rather than crates.io** — `rustysynth`, for SF2 modulators,
  ...
  build changes: `Cargo.lock` pins the commit exactly as a registry checksum did, and the fork's
...
**It rewrites `Cargo.lock` on the way through**, dropping the `source` line so the entry names a
bare `rustysynth 1.5.0` with no origin at all — which is a lock file that pins nothing and that
`--locked` will then reject everywhere. Put it back with a plain `cargo check` once you are done
(`git checkout -- Cargo.lock` restores the *previous commit's* lock, which is not the same thing if
the dependency line itself is what you are changing).
...
the binaries too, so the file a `cargo km-stream` left there is overwritten by one with the icon
compiled out — and a streaming run from it is silent about the difference, the icon being a blemish
rather than a fault.
...
- **`tools/cmd/assets` costs out of proportion to its 271 tests.** It builds at `opt-level = 1` with
  its dependencies at 2, and it shares no artifacts with the root build, so touching one of the nine
  `crates/` that `km-admin` depends on costs 51s of recompilation there on top of the root workspace's
  own.
...
files have to repeat it because no format lets them derive it, and `tools/dev/check-toolchain-pin.sh`
...
- **`rustup target add` is per toolchain, and a bump silently loses the targets.** Android and iOS
  standard libraries are attached to the toolchain they were installed for, not to the checkout, so
  the first Android build after a bump fails its target check in
...
- **The Debian image rebuilds itself.** `tools/platform/linux/image-tag.sh` hashes
  `rust-toolchain.toml` into the image tag, so a bump produces a new tag and the next `deb.sh` or
  `check:linux` re-runs rustup and `cargo install cargo-deb` — several minutes, once. Without that
  hash the old tag would still name an image with the old compiler baked in.
...
for the same reason the toolchain pin is: `tools/cmd/assets` is a second workspace, `exclude`d from
the one above, and nothing is inherited across that boundary.
...
day. [`RELEASE.md`](RELEASE.md) is the order the whole cut goes in, and that is its first step
because the range it is written from is easiest to read before the tag exists.
...
- **The version is a folder name before it is anything else.** A staged folder is
  ...
  program, and the one `tools/dist/bin.sh` lacked staged `km-admin-0.1.0-…` and then said nothing was
...
`git grep -F <old version>` afterwards is the check that exists, and it finds prose too: a kernel
version in a sample journal line, a date-stamped record in an architecture note and a test fixture
are all coincidences that must not move. **`CHANGELOG.md` is the
deliberate one**: every number in it is a record of a release that happened, so a substitution
through that file rewrites history rather than the version.
...
rather than looking for a system copy, and SDL3 is a C library whose build system is CMake; SDL3_ttf
then does it a second time for its vendored FreeType. The payoff is a self-contained binary and no
`brew install sdl3` for anyone who builds this; the price is CMake plus a C toolchain on every desktop
platform.
...
The variable is harmless when nothing needs it, and it is set in three places for the three ways a
build starts: the workflow's top-level `env:`, because the runners ship CMake 4 on all three
platforms; the Taskfile's global `env:`, so `task check` on a Mac needs no prefix; and the hand-typed
`cargo` above, which is the only place it is spelled out.

**SDL needs more X11 development packages than an obvious list carries**, and the lesson is in how it
fails: SDL's configure aborts on the *first* missing required dependency and names only that one, so
fixing them as they surface costs a full CI round trip each. **Install SDL's whole documented list,
not the package the error names** — "SDL builds" is the property needed, and SDL decides what that
takes. Names are verified against the distribution's archive before being added, because a typo fails
the apt step instead, which looks like an unrelated problem.
...
grant, a terminal application carries its own. So `task dist:bin` and the setup drivers go through in
...
Both are macOS-only and **neither could be caught by the checks that actually run**. They have no
root cause in common; what they share is the shape — *the platform makes an assumption false, and the
code that depends on it reports the opposite of the truth rather than failing loudly.*
...
then swept away by the operating system can never be forgotten.** The install-time guard at the other
call site is unaffected — *there* the file exists, so both sides canonicalise — **which is what makes
the answer depend on when it is asked.** The test pins both an existing and a vanished path, because
one of them passing proves nothing about the other.
...
**The general rule: never pipe one of this project's binaries into `grep -q` under `pipefail`.** Piping
from `find`, `otool` or `pkgutil` is safe by luck rather than by design — they die quietly on
`SIGPIPE` and their output is short enough to be written before grep exits — but a Rust program turns
...
Two environment variables, and **both** are required or the build fails in ways that look unrelated to
either: `FFMPEG_DIR`, a directory holding ffmpeg's `include/` and `lib/`; and `LIBCLANG_PATH` —
**mandatory, not a convenience**, because `ffmpeg-sys-next` runs bindgen at build time and ships no
pre-generated bindings.

**Neither is exported by hand.** `fetch-ffmpeg.sh` installs both and writes the paths into cargo's own
`[env]` table, between markers it rewrites in place. Recording it *there* was arrived at by
elimination:

- Cargo applies `[env]` to every build script it runs, so the setting reaches the build from any
  shell, from a script, **and from an IDE that invokes cargo itself**. An IDE launched from a desktop
  shortcut never saw an `export`, which is why "works in my terminal, fails in the IDE" is the normal
...
API — never `setx`, which truncates at 1024 characters, and never by echoing `$env:PATH` back to User
scope, which is the merged machine+user value and would copy the system `PATH` into the user's
permanently. **Until a shell is restarted, a video build compiles and then dies at run time** with
`STATUS_DLL_NOT_FOUND` — observed, not predicted. Nothing is wrong with the build; the test binary
simply cannot find the DLL.

**On Linux neither variable is set, deliberately**: pkg-config finds the libraries and clang-sys finds
libclang through the distribution's own layout, so setting either would replace a correct answer with
a guess. **On macOS neither has to be set by hand** — the bundle script resolves both itself and fails
naming the missing one rather than letting bindgen fail obscurely.

**The crate version does not pin the ffmpeg version.** `ffmpeg-sys-next`'s build script reads the
installed headers and enables version cfgs from a table spanning ffmpeg 3.0 to 9.0, so one crate
version compiles against 7.1.5 and against anything newer. **The obvious assumption — that crate 9.x
needs ffmpeg 9.x — is wrong, and believing it would send the whole dependency choice the wrong way.**
...
the **workspace at all** — which is not what this repository promises, not what CI installs, and
exactly the shape where an IDE that builds the whole project fails while building one package is
fine.

So: one default-off feature on the crate itself with a `cfg` at its root, and a bare workspace build
compiles it to an empty library in about a second. **Consumers do not name that feature**: the root
manifest puts it on the workspace dependency entry, so the three crates that take `km-video` write
`{ workspace = true, optional = true }` and still get a real one.

### A `#[cfg]` block nothing compiles is a block that rots silently

A build without the feature does not typecheck the `#[cfg(feature = "video")]` block, so a refactor
that converts every call site except the ones inside it breaks `--features video` outright **while
every ordinary command keeps passing** — they are all feature-explicit and none of them turns video
on.
...
on a machine lacking it, and `--browser` could not rescue it, the failure being in the dynamic loader
...
- **The error names a `.pc` file**, so it reads like a machine that needs a package installed rather
  than a feature that should never have been on. **The obvious remedy is the wrong one, and it is one
  `apt-get install` away from being taken.**
...
`cargo km-build`/`km-test`/`km-lint` are the video commands and the `-no-video` twins are beside them,
because `check.sh` — the script this project actually relies on — runs the video list.
`km-build-no-video` is a bare `cargo build --workspace` and by the rule at the top of
`.cargo/config.toml` needs no alias at all; it exists so `task build:no-video` forwards to one,
because **a task spelling a cargo line nothing compares is exactly the drift that file prevents.**

**Two things this is not.** Not a change to the cargo `video` feature, which stays off by default, so
a fresh clone still compiles the workspace with no C toolchain — nor to CI, whose runners have neither
ffmpeg nor libclang. And it stops short of the per-tool aliases, because `km-pack book` and
`km-pack check` open no song bytes and want no ffmpeg.
...
rustup and the `cargo install` and everything after — several minutes, for a change that has nothing
to do with either.
...
the build script, and a toolchain only governs what compiles the app.
...
workspace member, so `--workspace` compiles it either way, and it keeps its ffmpeg dependency behind a
feature of its own — a bare workspace build compiles it to an empty library.
...
- **Do not type the feature list out.** It lives in `tools/setup/features.sh`, is spelled a second time
  in `.cargo/config.toml` because a cargo alias cannot source a shell file, and
...
Worth running before a push: `Path` treating `\` as an ordinary character, a missing system library and
a wrong `#[cfg]` are the three failures that only appear off Windows, and all three have happened.
...
exported and a build works from any shell and from an IDE that invokes cargo itself — the case an
`export` cannot reach. A real environment variable still wins. A project-local override file is **not**
an option: cargo's config `include` key is silently ignored on stable.

**An override bank is cached and never installed into `assets/soundfont/`.** Everything under
`assets/` is shipped by every carrier, so a 206 MiB bank left there would be added to every release.
...
`audio.soundfont`**, which every build on the box reads. A hard link where the filesystem allows one,
so a 262 MiB bank is not duplicated per worktree; a copy across volumes. The install is the part that
matters: the folder is what says which banks exist, so a bank left only in the shared download cache
would be named and then not found. A *different* bank already there under the same name is refused,
naming both, rather than being given a `-2` — two confusable rows in the picker are worse than a
refusal you can read. Nothing here touches `assets/` or `dist/`, so nothing here can reach a
release.

- **The bank is opened before anything is written**, and one that will not play is refused in the
  synthesizer's own words. Four of fifteen surveyed banks do not load at all, and the symptom of naming
  one by hand is a machine that comes up on a sine test tone with the reason in a log.
...
long enough that the first bank is no longer in your ear when the second one plays. The switcher is
...
- **`task soundfont:debug` can only reach the top of the table.** It keeps the first eight rows that
  are already cached, and the table is in rank order — so the slots are structurally always the
  highest-ranked cached banks. `:choose` is how you reach the other fifty-four: a checkbox list of
  every bank, opened with the current slots ticked, that fetches whatever is ticked and missing after
  telling you how much that is. Space ticks, typing filters, Esc changes nothing, and unticking
  everything empties the slots. A bank whose terms forbid mirroring is drawn but cannot be ticked,
  and saying so happens at the prompt — the list stays up and nothing else you ticked is lost. The
  list itself is `km-pick`, a hundred lines around `inquire`'s `MultiSelect` that knows nothing about
  SoundFonts.
- **The slots are in table order either way**, so the highest-ranked bank you tick is `Ctrl+2` and
  the two commands cannot disagree about what a slot means.
- **`Ctrl+1` is always the bundled bank**, whatever is in the slots, so there is one digit that
  cannot be misconfigured and a fixed reference to judge the rest against.
- **A non-empty list is the on-switch.** With no slots filled the keys do nothing and nothing is
  drawn; with any filled, the name of the bank in force is on screen for the whole session, so a
  recording of it says which bank you were hearing.
...
`display.fullscreen` defaults to off — a default reaches an install with no settings file, and so
does a checkout somebody has just built, so it could not serve both. What tells them apart is who put
the machine there, so the three setup programs pre-write `{"display": {"fullscreen": true}}` where
there is no `settings.json`, beside the `first-run-soundfont.json` they already place. `--fullscreen`
and `--windowed` move one run either way and write nothing down; `F` still toggles. See `The setup
programs pre-write a settings file` in `docs/decisions/distribution.md`.

**`--stream` is the cheapest proof the screen draws on a platform.** It needs no display server, no X
and no Wayland: start it, fetch `/stream/live.m3u8` and decode a frame, and the fonts, the CJK
fallback, the layout, the wallpaper and every branch of `draw()` have been exercised — which is why
it runs over SSH on a box with no monitor. What it cannot say is which backend SDL picks for a
**window**, and that wants a real session.
...
is the `tray` feature, which only the Windows and macOS staging scripts name, so a run from a
checkout puts none up.
...
- **`--first-run-soundfont` writes a request and fetches nothing.** It takes `recommended` or a bank
  id, and the *next* start downloads it, checks it plays and chooses it — three starts try before it
  gives up. It is what the Windows and macOS setup programs' tick box writes, and the way to get the
  same behavior on the appliance, which has no tick box. `--show-paths` names a pending request;
  deleting that file, or choosing any bank yourself, calls it off.
- **`--set-debug-soundfonts` takes `<path>`, `<path>=<name>` or `<path>=<name>=<level>`**, in slot
  ...
  does not know.
- **`debug.wallpapers` is the same idea for pictures**, and has no flag: a list of extra images or
  `.zip` packs in `settings.json`, shown **on top of** whichever wallpaper folder the rules chose.
  `--show-paths` names the entries on `debug wall` lines and says which are missing. Editing the
  list and pressing the wallpaper-advance key picks it up with no restart, because the folder choice
  and the extras are both re-read at every rescan. A path that is not there simply never appears —
...
- **`--set-debug-packages` is how you install a `.kmpkg` that is not in a folder the machine
  scans**, and it is *added to* those folders rather than replacing them. Every package is opened
  before anything is written, and each path is made absolute. It is the successor to
  `settings.packages`, and it lives behind `debug.` because of how it fails: each entry is replayed
  at every pass, so a file moved or tidied away is a fault reported at every pass until somebody
  clears it. **For a library that lives somewhere else, use `settings.package_dirs`** — naming a
  folder cannot rot the way naming a file does. `--clear-debug-packages` empties the list and
  **deletes nothing**; anything that was also in a scanned folder stays installed. `--show-paths`
  names the entries on `debug pkg` lines, and says which of them are missing.
...
- **The machine is quiet by default**, plain `info`. `--frame-stats` is the only thing that builds the
  meter (`KM_FRAME_STATS=1` too, for a systemd unit); `RUST_LOG` overrides the `-v` ladder and does
  *not* bring the meter back, and neither does `logging.level`.
- **On a retail Android TV `--frame-stats` cannot be reached, but `F12` can — so the meter is
  ...
  vector, so `KM_FRAME_STATS=1` is the only route, and setting an environment variable for an app
  needs the `wrap.<package>` system property, which a retail build's SELinux policy refuses
  `adb shell` outright. It fails as *"Failed to set property … See dmesg for error reason"*, for a
  short package name as readily as a long one, so it is the policy and not the 32-character name
  limit.
  ...
  **What closed the gap is that the meter stopped depending on the flag.** `F12` builds it too, and
  ...
  measuring frame timing there needs no throwaway build with the flag forced on, and the panel is
  the *better* instrument anyway, because it puts the numbers where the person who can see the
  stutter is sitting.
...
- **A video song that could not be decoded in real time warns without any of that**, which is what
  makes the gap survivable. `starved_ms` — how long the sound ran dry, which is how long the song
  visibly and audibly stopped — is reported at `warn` when the song ends, on a default `info` machine
  with no flag set, because it is a fault rather than a measurement. A healthy song reports nothing.
...
- **`--log-keep <count|all>` says how many to keep, and turns the file on by itself.** `all` is for a
  machine being worked on: the names carry the date and time each run started, so a folder that is
  never pruned is that machine's whole history in the order it happened. `KM_LOG_KEEP` does the same
  with no command line, and the package builder, the offline remote and `km-admin` read it too.
- **`settings.json` says all of it permanently**, for the programs nobody types at. Four keys in one
  ...
  ```
  **Three programs read that section and it is the same section in all three** — the machine's
  `settings.json`, the package builder's in its config folder, and `km-admin`'s in its data folder.
  The offline remote keeps no settings file, so its flag and `KM_LOG_FILE` are the whole of what it
  reads.

  `level` takes what `RUST_LOG` takes, from a bare `"debug"` to a list naming targets, and it
  replaces the `-v` ladder rather than moving along it — which is how it says the thing a rung
  cannot. `keep` takes `"all"` or a number. `ecapplog` takes `true` or an address. **A value that
  will not parse leaves the built-in behaviour standing and says so in the log**, rather than
  stopping the program or being guessed at. The section is **not** under `debug`, deliberately:
  `debug.enabled` publishes a passwordless copy of the API, and asking for a log must not be a way
  to arrive at that.
...
- **`KM_ECAPPLOG=1` turns that on for every program at once**, which is what a checkout wants while
  this is being worked on: put it in `.cargo/config.toml`'s `[env]` block beside `KM_NO_MDNS`, and
...
It writes a second set too — the files that must be *refused* — which is what to point a tool at when
you want to see what it says about a bad file.
...
The paper half of a karaoke machine: four columns, alphabetical by artist inside a section per
language, modeled on the book a commercial machine ships in a ring binder.

- **It reads the catalog as it stands.** Packages are indexed when the machine *starts*, so a
  `.kmpkg` dropped in since the last start is not in the book — and the symptom is indistinguishable
...
because
the build compares what you wrote against what the file says and records the difference as a
...
- **A build refuses a song with no language.** Most MIDI files classify themselves; what is left is
  ...
  the honest answer.
- **MP3+G pairs are packaged by the same command, with no feature and no ffmpeg.** Nothing is ever
...
A build measures every video and MP3+G song's loudness and writes it into the manifest, so the
machine can bring it down to the level its bank renders MIDI at — see
...
of the events as it starts, in a fifth of a millisecond, and moves it to the same place — up as well
as down, which is what answers a file that plays too low. So a package built before any of this, and
a file played straight from disk, are both levelled with no rebuild. `audio.normalize_midi: false` in
`settings.json` turns it off, separately from `normalize_media`. See
...
- **It costs a full audio decode per media song, about 0.6 s each, and that is a release figure**, and
  a video's picture is never decoded. `--no-loudness` is for somebody iterating on a description who
  does not want to pay it each time; a song built without it plays unlevelled, exactly as one built
  before this existed.
...
- **`reanalyze` is the migration path, and it needs no sources.** It measures the media *inside* an
  ...
  suitability as it always did, so its `--no-loudness` skips only the measuring.
...
- **`--register` is per-user and needs no elevation.** It records where the exe is *now*, so re-run it
  if the folder moves. On macOS the type is declared by the bundle and the flag only nudges
  LaunchServices.
...
- **The Settings page's Discover button lists machines and never sets one** — this tool installs
  ...
  wrong machine.
- **`--backup` writes only what a person typed**, which is the half a re-scan cannot rebuild: titles,
...
- **The flag takes a path verbatim; the Settings page's default carries the moment** —
  `km-package-builder-20260909T140233Z.kmbackup.json` in `_kmbuild-data`, so a second backup sits
  beside the first rather than replacing it. The restore box suggests the newest one it finds.
...
- **`KM_PACKAGE_BUILDER_RECENT` names this run's recent-folder list**, and set-but-empty keeps none —
  ...
  config folder: twelve entries of somebody's own curation, never pruned of folders that have gone
  away, and the startup reopen takes the first of them. **A screenshot run, a smoke test or a
  scripted build sets this**, so that naming a throwaway folder does not evict a real corpus. There
  is no flag, because the runs that must not write here are the ones a script starts.
- **`KM_PACKAGE_BUILDER_PASSWORDS` names this run's remembered machine passwords**, with the same
  three states. Every other run writes `machine-passwords.json` beside the list above — a machine id
  to the admin password somebody ticked "remember" for, `0600` where the platform has it, and deleted
  rather than emptied once the last one is forgotten. **A run that talks to a machine sets this too**,
  for a stronger version of the reason above: a smoke test that wrote here would be putting a
  credential into a store its owner believes they control.
...
- **It is the other half of `/admin/`, and it *serves* `/admin/` too.** The machine takes
  files and cannot go and find one: it may have no internet, should not hold somebody's Pixabay key,
  and has better things to do than a hundred JPEG decodes. This is what produces the file — and the
...
- **Everything is under `/admin`, and `/` redirects to the front door.** That is where the shared
  ...
  `/admin/sound/fetch`, the last two reached by a link from the shared Pictures and Sound tabs — the
  searching is a page under its tab rather than the tab itself, which is a consequence of sharing the
  markup and is argued in `docs/decisions/distribution.md`. **The redirect is temporary and a shell
  opens the door itself**: a browser keeps a permanent redirect and follows it without asking again,
  which would make where the door is a promise every later build has to honour.
- **It opens on the front door, and that is where a machine is chosen.** A page with no tab strip:
  the machine it last chose, what is advertising itself over mDNS, a box for an address and a box for
  the password. Every launch, and the tabs are not drawn until a machine is picked — whichever row
  you press is remembered in `<data-dir>/machine.json`, so `--machine` is for a machine broadcast
  cannot reach, or for pointing one run somewhere else without disturbing what is remembered.
  **Nothing is set until you press a row**, the rule `km-package-builder` keeps and for its reason:
  ...
  one on the wrong machine. Two things that look like faults and are not: a machine serving on
  `127.0.0.1` advertises nothing at all, so a loopback test instance never appears; and a machine
...
- **The banks come from the same table the machine compiles in**, `km-banks`, verified against the
  digest it pins. A machine with its own internet connection can fetch these itself — this is for
  the one that cannot, and for the television box with no shell.
...
- **Everything it *makes* is written to `--data-dir` as well as being sent**, so a machine that is
  switched off is a delay rather than a dead end. A file you already have is staged under
...
- **`--lan` is off by default**, which is *not* the choice the machine's own remote makes: that one is
  open to the house because anybody in the room should be able to queue a song; this one holds one
  person's favorites.
- **Queueing works against any machine**, which was not always true: everything the offline remote
  drives is public, so a machine having a password costs it nothing. It still cannot reach anything
  under `/api/v1/admin/`, and has nowhere to type a password — but nothing it offers is there.
...
**The picture examples are judged by eye**, like the screens: a palette read wrong, a tile bitmap
reversed or a baseline off by a row all produce output that still *looks* like output.
...
reports the pitch bend range and tune each channel ended on, and it answers the question a WAV
cannot: whether the file's RPN setup was acted on or discarded. A part written as bends against a
twelve-semitone range and played at the default two is out of tune by a different fraction on every
note, and the dump says `12.00` or says nothing.

**The three SoundFont tools answer three different questions, and only the last one is slow.**
`bank_info` prints one file's contents and what the synthesizer dropped to load it — the columns
[`soundfont-banks.conf`](crates/machine/km-banks/data/soundfont-banks.conf) carries, which until now
came from a tool that was never committed. `event_census` answers "how rare is that MIDI message,
really", which is the shape of question that had gone unmeasured and produced a wrong comment about
aftertouch. `fix_census` beside it answers the same shape of question about a *defect* — how many
files carry one this machine can correct — and it takes a **`stride`** because a bare limit reads the
first files the walk reaches, and a walk reaches them a folder at a time: on a corpus whose folders
came from different sources that is a figure about whichever few happened to be first. Every
twenty-fourth file spreads the same number of reads across the whole of it. It parses rather than
searching the bytes, which matters for exactly this question and in both directions: `B4 00 7F`
occurs inside track names and delta times where it is not a bank select, and a bank select written
under running status carries no `B4` for a search to find. Over the corpus the second outweighs the
first, so the byte search reads low. `soundfont-measure.sh` re-derives that table's spread, volume and **lufs** columns over
every bank in the asset cache: seven songs each, and it **never downloads** — a bank that is not
cached is named and skipped, since several of the pinned banks come from a slow archive host. It needs
`KM_CORPUS`, because the seven songs live in a local folder that no committed file may name.

**`lufs` is the one column of the three the machine acts on**: it is the reference a video or MP3+G
song is attenuated to, so a bank added to the table without one falls back to a constant and is
levelled slightly wrong. Its `Mean LUFS` output is that number. **Re-run it after a `rustysynth`
change and record the commit** — the branch can advance twice in an afternoon, and the way to know a
run is comparable with the recorded rows is that it reproduces their `spread` to the decimal.
...
It refuses three things that would otherwise be found only after publishing — a picture the page
...
and `--no-video` asks for the smaller build; a script that cannot find ffmpeg **stops before it builds
anything**. They are quiet, and `-v` watches one — **a step that fails replays everything it held
back.**
...
- **The remote's two are independent carriers**, not smaller selections of the other two: their own
  `AppId` on Windows and their own receipts on macOS, so one computer may hold both and removing
  either leaves the other. The Windows build refuses when the two `AppId`s match.
...
- **All four are unsigned by default**, so a recipient sees SmartScreen or Gatekeeper. Signing the
...
`Depends` list is *complete* — which only holds because the image starts with nothing. The tarball's
...
so neither is ever implied. **`local/`, `scratch/` and `.wpcache/` survive every setting.**
...
- **`ARM64=1` is not a thing to ship**: every Google TV device runs a 32-bit OS and loads
  `armeabi-v7a` alone, so an APK without it installs on a phone and fails on a television.
...
- **The two iOS builds are the only tasks here that cannot run on Linux or Windows.** They carry
  ...
  so a green line on another host says nothing about whether the app compiles.
...
**[`DEPLOYING.md`](DEPLOYING.md) is the reference** — the requirements at both ends, the display
manager that causes a black screen, and why the journal is read with `-t` rather than `-u`.
...
terms forbid distributing content as a wallpaper — so such a pack is never committed and never staged
into a release, whoever types `--zip-dest`.

In the second workspace, `tools/cmd/assets`, so every command names its manifest — the **workspace
root**, with `-p` picking the program, which is what keeps one spelling working as members are added.
Keys come from the environment and never from the config file.
