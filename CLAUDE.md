...
files with their audio as the only song sources, a native cross-platform app (Windows/macOS/Linux
first, Android second), synced word highlighting, and an HTTP API for search, queueing and control.
...
2. **A new product decision gets an entry in [`docs/decisions/`](docs/decisions/)**, in the file for
   its area — and so does a changed requirement, because a requirement and the decision that serves
   it are the same entry. Do not record either only in an architecture note. That entry is the
   convention most worth protecting.
...
4. **Research notes are not decisions.** Nothing in one is committed until it appears in
   `docs/decisions/`.
5. Do not create additional plan or status documents. Reference documentation is not caught by this
   rule — `README.md`, `BUILDING.md`, `CONTRIBUTING.md`, the per-port READMEs, `icon/README.md`,
   `docs/learning-rust.md` and `docs/HISTORY.md` say what *is* or *was* true, where a plan says what
   *will be*.
6. **Prose states the rule and the reason, and never how the rule was arrived at.** No "used to", no
   ...
   appositive tail restating the clause before it. **This governs code comments, commit messages and
   everything the project publishes** — the release page, the changelog, the site, the words inside a
   program — as much as documents, and it is the rule most easily broken by somebody writing up a
   change they have just made. The full form, with the list of what creeps back, is
   [`How a document in this repository is written`](docs/decisions/repository.md#how-a-document-in-this-repository-is-written);
   `task lint:prose` catches the shapes that have one, in the lines a branch adds and in the messages
   it commits them in. **A commit's subject states the rule that holds after the change and its body
...
- **A song is a MIDI file, a video file, an MP3+G pair, or an UltraStar `.txt` with the MP3 it
  ...
  in it. An UltraStar file is read for its timed words only; its pitches are discarded. **CD+G as
  a *disc* format is not supported** — `.bin`/`.cue` images and raw subcode rips — because there is not one of them in the
  corpus: the file pair is what the world actually trades.
...
- **No audio-file pitch shifting.** An MP3+G song answers a key change with a 409, exactly as a video
  does, because there is no key to change and never will be.
- No in-app microphone DSP — mic audio is mixed in hardware.
- **No shaped scripts — Thai, Arabic, Indic — and no right-to-left.** These need a shaper, and
  ...
  reordering or joining, so they draw from a fallback face with the shaper still off, and the
  television does draw them.
- **No removable-media modeling** — no mount detection, no eject, no "volume absent" state. A folder
  that is not there is a folder that is not there. This does *not* stop a library living on an
  external drive named in `package_dirs`; what it refuses is the machine reasoning about whether a
  missing folder was removed or merely unplugged. **The catalog reconciliation leans on it**, so
  adding removable-media support reopens that rather than slotting in beside it.
...
- **How large the corpus is stays out too.** A file count, a song count, a row count or a database
  ...
  or the percentage the measurement turns on; a sample size — *4,000 files sampled from the local
  corpus* — is a number that stays, because it names the work rather than the library.
- **It cannot see a brand name, and it cannot see a count.** A model number, a first-person aside, or
  ...
  audio equipment fourteen times and passed every check for months. Six digits have none either. New
  prose needs a human pass.
...
- **Nothing may assume cargo builds into `target/`.** The directory moves. Ask cargo, through
  `dist_target_dir` in `tools/dist/common.sh`. Spelling `target/release/x` produces the worst
  failure this repository has had from one line: cargo prints `Finished` and the next line says the
  executable was not produced.
- **No test may bind a non-loopback address.** On Windows a test binary's path carries a build hash,
  so each rebuild raises a fresh firewall prompt and leaves a dead rule behind — 60 of them
  accumulated once. `km-api`'s `on_ephemeral_port()` is the pattern.
- **Pair `--api-bind` with `--data-dir`**, or two machines on two ports still share one catalog,
  one packages folder and one settings file — which is not two machines. Prefer the full loopback
  form `--api-bind 127.0.0.1:<port>`: a listener on `0.0.0.0` raises a Windows firewall prompt per
  program and port.
- **On macOS prefix a bare `cargo` with `CMAKE_POLICY_VERSION_MINIMUM=3.5`** — CMake 4 rejects the
  ...
  the only case left that does — the one point where the two spellings stop being interchangeable,
  and a deliberate exception argued in
...
- **`master` takes pull requests only, and requires one check, `CI ok`.** A new CI job goes into
  `ci.yml` and under `ci-ok`'s `needs`; a workflow of its own with path filters would never report
  on the pull requests it skips, and those could never merge.
- **`gh pr merge` takes no `--delete-branch` here.** The repository deletes a head branch on merge
  itself, so the flag reaches only the *local* branch — and deleting that means `gh` checks out
  `master`, which the main checkout already holds. From a worktree, where the work is, that is
...
new one is often started while another is mid-edit — so a session cannot know it is the only one
running, and the answer can go from yes to no while the work is in progress. Size is not what the
rule is about; the shared path is.
...
The rest — why `target/` is deliberately not shared, the Docker build-cache collision, and what a
worktree carries that `git worktree add` does not — is in
[`CONTRIBUTING.md`](CONTRIBUTING.md#branching-and-worktrees).
...
binaries, **`tools/`** is everything you type or run, **`ports/`** is the native application shells,
**`icon/`** and **`site/`** are the published identity, and **`docs/`** is the table above.

**Each of those carries its own `CLAUDE.md`** holding the part that stops a mistake — the second
cargo workspace under `tools/cmd/assets/`, what `km-pack build` takes, why `fixtures/` is empty. They
are not loaded now; they arrive when you open a file there.
