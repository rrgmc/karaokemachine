...
**Give `--arm64-only` to both, when running them by hand.** Skipping an ABI in `build.sh` leaves the
library an earlier both-ABI build put in the target directory. A staging step that copied whatever it
found would then put a stale one in the APK. Arm64 built this minute would sit beside an armv7 from
weeks ago, in a package whose whole point was to leave armv7 out. `stage.sh` removes any ABI directory the run
did not stage, which keeps `jniLibs` a picture of this build rather than of every build. The
machine's version of this note matters more, because a television runs the leftover.

Out comes `app/build/outputs/apk/debug/app-debug.apk`, **25 MB debug and 12 MB release**. The two
libraries are essentially all of it either way (16.1 + 9.2 MB debug, 6.8 + 5.3 MB release). Gradle
strips them, so these are far below what cargo leaves in `target/`.

There is no asset step and nothing to fetch first. `km-remote-pages` compiles its templates, its
stylesheet, its script and its icon in, and serves them from routes. Unlike the machine there is no
`MANIFEST` and nothing unpacks on the device. No SoundFont or wallpaper pack has to arrive before a
build will work.
...
Worth knowing when editing any XML here: **a `--` cannot appear inside an XML comment**. The resource
compiler reports it as a bare parse error, and it has now cost time in this project twice.
...
side's own HTTP to the machine is a native socket, which that file does not police, so nothing needs
widening.
...
Rust side is unchanged. Dropping it while backgrounded is correct rather than a compromise. A browse
then finds nothing, and the server's own recovery logic answers "stay where you are".

**The camera, asked for twice over.** The share page's scanner needs `CAMERA`, and somebody has to
answer Android's own prompt before the *page's* request can be granted. That is why
`onPermissionRequest` holds the request and `onRequestPermissionsResult` answers it. Android never
asks again for a camera denied twice, so the shell says where Settings is, and the page always has
its paste box.
...
`android.hardware.camera` **and** `android.hardware.camera.autofocus`. Any of those would filter the
app off a device that lacks it. This is a remote whose whole job works with no camera and no Wi-Fi
hardware named. The manifest declares all three only to un-require them. Check it on the built
...
**No wake lock, and the screen sleeps on its own timeout.** This port holds no `WAKE_LOCK`
permission, no `PowerManager`, no `getWindow().addFlags(FLAG_KEEP_SCREEN_ON)` and no
`setKeepScreenOn`. The two `getWindow()` calls in `MainActivity` are the inset controller and the
status-bar color.

**Note the contrast with the machine's app**, which does hold the screen on, and takes it from SDL
rather than from anything in its manifest. See *The screen staying on is SDL's* in
`ports/machine/android/README.md`. That is a box under a television with a song on the screen. This
is a phone somebody put down. A remote that stopped a phone sleeping would be a battery fault wearing
the shape of a feature.

**What that costs is the paragraph above it**, and the two belong together. No service holding
anything and no wake lock leaves Android free to freeze this process the moment the screen goes off.
Everything in `Coming back to a page is a reason to try the machine now` (in
`docs/decisions/remotes.md`) is the bill for these two lines, paid on the recovery side. That is
where it belongs. The alternative answers "the remote lost its connection while I put the phone
...
restoring one uses an `<input type="file">`. `MainActivity`'s chrome client therefore answers a
...
both answers, including `results.length == 0`. That is how Android reports its own prompt dismissed
by a tap outside rather than answered either way.

Three things here differ from the Go counterpart deliberately. `MainActivity` builds the file-chooser
intent by hand as `*/*` rather than from `params.createIntent()`, so the page's `accept` list does
not become a MIME filter. Drive and Dropbox report their own types, and a filter makes a backup that
is plainly visible impossible to pick. `setType` for a save comes from the listener's own `mimetype`
argument rather than a hardcoded one. The copy loop is written out because `InputStream.transferTo`
is API 33 against `minSdk 26` with no desugaring here. That would be a `NoSuchMethodError` on exactly
...
The release build type takes the key `KM_ANDROID_KEYSTORE` names, and the debug key where it names
none. A fresh clone therefore builds with nothing set up. Check what a build actually used rather
than assuming:
...
different key over an existing one. A device holding a build from before the release key therefore
uninstalls to take one, and an uninstall deletes app-private storage, where `favorites.sqlite` lives.
...
On a **Galaxy S23, Android 16, arm64**, against a machine advertising on the LAN:

- discovery on the first browse, the catalog mirrored, the event stream connected and the page's dot
  green;
- the page rendering clear of the status bar, with the tab bar reaching the bottom edge;
- backgrounding and returning keeping the same process, the same port and a live page with **no
  reload**, which a forked-child design cannot do;
- back at the root, the server shutting down cleanly.

That run found two bugs, and a desktop could reach neither. The multicast lock was taken in
`onStart` while the server starts in `onCreate`, so the first browse ran without it. The
"No karaoke machine found" screen did not notice the background browse succeeding twenty seconds
later. It left a false sentence in front of a working remote.
...
the first launch. The server came up, it found the machine, and it mirrored the catalog:
...
says they are *present*. Only an Activity starting says the runtime **resolves** them. The failure it
guards against is a JNI export left on an old symbol after the namespace rename. That compiles,
...
`km-remote-pages` is the tag, which is what makes it filterable beside the machine's
`karaokemachine`. A release build says `info`, and a debug build adds this application's three
crates. An Activity has no `RUST_LOG` to set and no command line to put a `-v` on, so the build type
is what asks.
