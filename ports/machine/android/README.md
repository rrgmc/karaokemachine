...
before Gradle — the cargo half, which needs no JDK. `RELEASE=1` reaches all four, giving optimized
native libraries inside a release APK; `ARM64=1` is `--arm64-only`
and **also reaches both** for the reason below; `NO_VIDEO=1` is `--no-video`. `task ffmpeg:android` is
the once-per-machine step.

**`--arm64-only` has to be given to `stage.sh` as well as to `build.sh`**, and typing it on the first
line alone is the trap. Skipping an ABI does not delete the library an earlier both-ABI build left in
the target directory, so staging copies that one and the APK carries it — and here that is not merely
wasted space, because a television loads `armeabi-v7a` and nothing else. The APK would then be
current on a phone and months old on the television. `stage.sh` now removes any ABI directory the run
did not stage, so the two halves cannot disagree; the warning at the end of `stage.sh` depends on
that, having previously been silenced by exactly such a leftover.
...
build` decision in `docs/decisions/`. It needs `tools/port/machine/android/ffmpeg.sh` to have run once per ABI, and
`build.sh` refuses to start rather than quietly producing the APK that lists video songs and cannot
read them. That script cross-compiles the same pinned LGPL ffmpeg 7.1.5 the macOS bundle and the
Linux tarball carry, into `~/.cache/karaokemachine/assets/ffmpeg-android/<id>/<abi>/`, and needs
nothing installed that an Android build did not already need: the NDK supplies clang **and** a GNU
make, including on Windows. It also writes two `force`d entries into cargo's own `[env]`, without
which bindgen cannot find clang's builtin headers; the header of the script says why, and why the
obvious fix for that is a trap. `--no-video` skips all of it and stages no ffmpeg libraries.
...
app-private storage on first run — see its module docs for why a `MANIFEST` is needed and why no JNI
is. Skip the step and the machine still starts, on its test tone with a system font and a generated
gradient.

Out comes `app/build/outputs/apk/debug/app-debug.apk` — about 78 MB with video and the wallpaper
pack, of which the four ffmpeg libraries across both ABIs are only ~5 MB. The SoundFont (31 MB) and
a wallpaper pack (19 MB) are most of it; both are assets, and `assets.sh` prints the total.
...
major version 69`, and Android Studio's bundled JBR *is* 25 — so the JDK that is already on the
machine is the wrong one. `winget install Microsoft.OpenJDK.21`. A Gradle toolchain does not help:
...
Worth knowing when editing the manifest: **a `--` cannot appear inside an XML comment**, and the
manifest merger reports it only as `Error parsing AndroidManifest.xml` with no line number unless you
pass `--stacktrace`. Writing out a `cargo … --example …` command in a comment is what tripped it.
...
resizable — so it picked `SCREEN_ORIENTATION_FULL_USER` and a Galaxy S23 started portrait, at
...
`View` attribute and an `<activity>` element has no such thing, so it is silently ignored while SDL
quietly does the job. A line that appears to cause the behavior it is merely adjacent to is worse
than no line, because the next person to change the screensaver hint will edit the manifest and watch
nothing happen.
...
JBR shipped with a current Android Studio is JDK 25, so a JDK **17 or 21** has to be installed
separately:
...
- **`armeabi-v7a` decodes video on a Streamer.** The ABI where the 32-bit surprises live (the
  `LIBC_N` link, the Vulkan stub's 64-bit size assertion) neither fails at load nor at play: a
...
- **Decode headroom on the Streamer's 32-bit Cortex-A55 is measured and software decode wins.**
  1080p30 H.264 High at 926 kbps costs **80% of one core** single-threaded — the whole margin against
  a 250 ms ring, and a 273-second song stalls: 871 ms of silence and a ~4.5 s frozen picture. **Frame
  threading is what answers it.** The same file, on the same device, reports `threads=5 kind=Frame`
  and plays all 273 seconds with `starved_ms` **0** and no warning at all. The numbers and the method
  are in
  [`docs/architecture/video.md`](../../../docs/architecture/video.md). So the shipped build stays on
  software decode and `h264_mediacodec` stays unselected — and nothing measured since argues for it.
  **A phone settles nothing here** — a Snapdragon 8 Gen 2 does not notice 1080p30, and reading its
  success as evidence about the television would be the whole point missed. The **synthesizer** on
  that same A55 has been measured, and is in
...
- **An MP3+G pair has now played on the television too**, so every song kind has run on this device.
  It is the cheapest of the three to draw — 0.3 ms a frame against the lyric ladder's few, because a
  CD+G plane is a texture upload with no text shaped at all.
- **The physical remote has now been pressed, and nothing here is unverified any more.** Arrows move
  ...
  joystick device is ever added. What a real press found that injection could not: the first D-pad
  press after the strip fades is swallowed waking it, and the idle number pad had to be switched
  back on for every Android — see [`The on-screen number
