...
installs it on the first `cargo` command, so there is nothing to select and `rustup update` is not
part of building this. Bumping it is a deliberate commit — the procedure, and the two things it
silently breaks, are in
[Bumping the Rust toolchain](BUILDING.md#bumping-the-rust-toolchain).

**Never use `--all-features` on this workspace.** It is not a style rule. The flag turns on
`km-package-builder/desktop`, whose `wry` links libwebkit2gtk — a library
`tools/platform/linux/apt-deps.sh` deliberately never installs, because a Linux build of that tool has
no window by design. So on Linux the one flag meaning "everything" asks for the one thing the
platform is designed not to provide. It survived for a long time because it works on Windows and
macOS, where WebView2 and WKWebView need nothing at build time — the machine it was typed on could
not see the fault.
...
Worth running: `Path` treating `\` as an ordinary character, a missing system library and a wrong
`#[cfg]` are the three failures that only appear off Windows, and all three have happened here.
...
editing one checkout clobber each other, and small work is exactly the work that gets done in place.
...
**A worktree lives at `.claude/worktrees/<name>`, inside the checkout**, because a worktree there is
inside the permission root a Claude Code session already holds and one beside the repository is not.
...
- **Every value in `.cargo/config.toml` is a string, and an array breaks every worktree at once.**
  Cargo reads that file from the checkout as well as the worktree and joins arrays where it replaces
  strings, so an array alias works where it is written and expands to `build … build …` everywhere
  else. `task lint:cargo` refuses one. A command that needs an argument with a space in it goes in
  `Taskfile.yml`, which Task reads from one directory and never merges.
...
every hash below it, which costs a reader more than the sentence saves.
...
**A clean run does not mean the tree is clean.** The script checks three shapes — a drive-letter
path, a private-range address, a named home directory — and the rule above forbids two more things
that have no shape at all. A hostname, a model number, an email address and a person's name read as
ordinary prose. So **new prose needs a human pass**, and it is not optional:

- The research note that named the owner's own audio equipment fourteen times passed every check for
  months, and was found by reading.
- `km-package-builder`'s `is_loopback` test used the appliance's real hostname as its not-loopback
  case — the "not as test data" clause, in the one place nobody reads for prose.
...
What is deliberately exempt — the published identity, generic OS paths, second-person "on this
machine" addressed to the reader, stock hardware class names — is argued in
...
Read it before deciding something is an exception.
...
**The code filter is written as exclusions**, so a new directory counts as code until someone lists it,
and a move makes a job run rather than skip. The `assets` filter is an inclusion and can fail open
after a move; the weekly run is its backstop.
...
bundled SQLite is minutes, and the whole value of a local check is that it finishes while you are
still thinking about the change. But **both mount the working tree at `/src`, and a git worktree
mounts at `/src` too.** Cargo's fingerprints record that path, so the volume cannot tell one checkout
from another and will hand a run artifacts compiled from different source at the same location.

**What that looks like is the reason it is worth a paragraph**: not a cache-shaped failure but a
*compiler* one, naming code you are looking at and can see is right — a missing field on a struct
whose definition has it, a wrong arity on a function that plainly takes one argument. The second
...
error at all, nothing downstream that would catch it**, and in the `.deb`'s case the deploy script
...
directory, so a hand-kept list has to remember that, and one that forgets **protects everything except
...
the release paths' skip unsafe — build from A, check from B, build from A again, and A's stamp still
matches so the clean is skipped over B's artifacts.

**The middle clause was added after walking into the case it excludes.** Recording does two jobs and
only one was intended: it makes a *later* release build clean, and it also **erases a mismatch that is
still outstanding**, because recording alone cleans nothing. The mirror image of the sequence above
was the live state of the box after a worktree was removed. So recording now leaves a stamp alone when
it names somebody else — and what makes that safe to reason about is the *direction*: **preserving a
mismatch can only ever cause more cleaning, never less.**

It also explains why the original reasoning looked complete and was not: "every writer records" was
derived from a single scenario in which the *other* checkout wrote last. **The rule it produced is
correct there and wrong in the reverse**, and nothing distinguishes the two without asking which
profile was written.

**The strictly more correct design, deliberately not taken.** A target directory per checkout would
remove the clean, the stamp, the race and the footgun at once. It costs a full cold build per checkout
and several GB apiece in a VHDX that never shrinks. **At two checkouts that arithmetic does not work.
Revisit it at three.**
...
- **The tarball's ffmpeg prefix.** One fixed path plus a stamp; a run whose stamp disagreed did
  `rm -rf` and rebuilt. With two checkouts that deletes a tree a peer may be compiling or running
  against, with no lock — and since each keeps judging the other's stale, **they ping-pong through a
  multi-minute rebuild every time either runs.** The prefix is hashed from the release and the
  configure flags, and the install goes through a staging directory and a rename so a tree appears
  whole or not at all. **Nothing deletes anything**, which is the property that makes it safe.
...
this, because the faults `check.sh` genuinely exists to catch do not usually present as a type error
about your own struct.

## The Taskfile

`Taskfile.yml` is the one listing of what this repository does. **It wraps**: every task is a line or
two of something else — a cargo alias, or a script under `tools/` — and the something else stays
...
reading a manifest means picking the right one of many lines. **The run tasks cannot use it: they need
the version *to find* the binary.** `cargo pkgid` needs no build and cannot disagree, because a clap
version is the cargo one.
...
- **`bash` on the Windows PATH is WSL's** — a different git looking at a different filesystem with no
  ...
  through its own embedded shell, `task` works unchanged from `cmd` and PowerShell — nothing has to be
  run from a Git Bash.
- **Task's shell is POSIX but its PATH is the host's.** There is no `sed`, `grep`, `awk` or `jq` on
  that box, so every variable is computed from `cargo`, `rustc`, `git` and shell builtins.
- **A glob expands with the host's separator**, so a `${e##*/}` strips nothing and the first
  `clean:old` **matched nothing while reporting success — the worst failure a cleaner can have.** That
  work lives in a script now, because Task's shell has no `rm` either.

**`clean:old` has no special cases at all, and that is the property to keep.** An entry is removed
only when its name begins with its app's own name followed by a version that is not the wanted one, so
anything unrecognized survives: the macOS bundle carries no version at all and never matches. It knows
one version, because there is one — `km-admin` and `km-wallpaper-pack` are in the excluded workspace and
followed the machine from 2026-09-04, where before that each needed an arm of its own here and in
`tools/dist/bin.sh`, and the arm nobody wrote is what broke `task dist:bin`.

**The coupling to watch**: the Taskfile and the clean script both reconstruct the release layout,
which is `dist_dir()`'s rule to own. **The Taskfile has to keep its own** — Task's embedded shell
cannot source a bash file — and so does the clean script, because every helper in the shared library
...
A curated corpus keeps the conclusions and not the evidence, so a build that decides something new
about a file cannot correct what is already stored. `km_suitability::ANALYSIS_REVISION` is written
beside every song and is how a scan knows to read it again; leave it alone after changing what a
scan would write, and the corpus keeps reporting what the build before you decided, silently, because
nothing about any file has moved.

It covers both crates that decide a stored column — `km-song` for the words, the flavor, the
granularity, the counts and the detected title, artist and encoding, and `km-suitability` for the
...
so tuning is caught even where no fixture moves. What it cannot catch is a behavior no fixture
exercises; that is what adding one to `km_song::testing::FIXTURES` is for.
...
**Cargo says `Finished` and the script says nothing was produced, in consecutive lines.** Both are
telling the truth about different directories. **The error names the assumption instead of the fact**,
which is why this reads as a broken build rather than as a wrong path.
...
things about that, three got wrong first:
...
- **It settles the excluded crate without a special case.** That crate builds into a target of its own
  — *unless* the variable is set, in which case it shares. **That is a fact about the environment
...
- **The value is JSON, so a Windows path arrives with its separators doubled** — and that is the
  default case, not a corner one. Left un-escaped it produced a path to nothing. It is normalized to
  forward slashes **only when drive-lettered**: a backslash is a legal character in a Unix filename.
- **It is not cached, and must not be.** Every caller writes `X="$(dist_target_dir)"`, a
  command substitution is a subshell, and a cache written inside one is gone before the assignment
  completes — **so an associative array would never once return a cached answer while its comment said it
  did.** Storing the result in the caller *is* the memoisation.

**The quiet failures are worse than the loud one** — a script that prints an empty list, or says
"run the build first" immediately after a successful build, gives **the wrong explanation a person
will believe.**

### Every path km-admin's own pages name is a route it mounts

**km-admin serves everything under `/admin` while its two routers declare their paths without it**, so
the prefix is written out by hand in every `src`, `action`, `hx-get` and `Location`. That is the cost
of mounting the shared page set at the same prefix the machine uses, argued in
[`docs/architecture/admin.md`](docs/architecture/admin.md), and **a path that drops the prefix fails
...
directory rather than a list, plus the consts that reach markup through a struct field. Each is driven
through the real router under a method nothing mounts, where **`405` says the path is there and `404`
says it is not** — so no handler runs, and the sweep needs no cache, no machine and no port.
...
a hash that changes on every relink — **so the rule never matches twice.** Each worktree has its own
build directory on top of that. One line left **60 inbound rules across 12 test binaries**, most
already gone from disk, before anybody traced where the dialog came from — **because the prompt names
a hash and no crate.**
...
Guarding that needs a socket whose address is not `127.0.0.1` — but **not** one the firewall cares
...
rots.** It is bindable inside the Debian container too, which was checked rather than assumed.
...
The rule as first written said only "bind", which is how the very next crate obeyed it and still
asked: its bind was loopback from its first commit; what it left alone was the *locator*.
...
browses from a machine listening only on loopback, **and that asymmetry is precisely where this fault
landed.**

That guard now carries more weight, because the advertiser is called from a task that ticks for the
life of the server rather than once at bind. **If anybody makes the address filter keep loopback, or
moves the emptiness check after the start call, that is the moment this stops being structural** — and
the symptom will be a test suite that opens multicast sockets on every developer's machine.
...
the first**, because it holds the socket open and goes on retransmitting rather than shutting down
after a timeout. It is split from [`Registry`](crates/machine/km-api/src/discover/watch.rs) for
exactly this reason: the registry is the merge and the policy and has no socket in it, so **every
test drives a `Registry` and nothing constructs a `Watcher`**. A test that needs one is a test that
wants a `Registry` and has not noticed.

**Two things now enforce the multicast half rather than only the prose.** `km_api::discover::daemon`
is the single constructor, and it answers `None` when `KM_NO_MDNS` is set — which
`.cargo/config.toml` sets, so a `cargo test` binary opens no multicast socket whatever it reaches
for. `tools/dev/check-mdns.sh`, run by `task check`, asserts that function is still the only caller
and that no test file names `Watcher::start` or `Mdns::new`. Both are a floor and not a pass: the
check matches by shape, and a variable is only as good as the one place that reads it. The standing
...
same run failed a second test whose budget was 600 × 10 ms — six seconds, against a `CONNECT_TIMEOUT`
of five and a `BACKOFF_START` of one, so a single slow handshake was the entire margin. A budget is a
deadline, and it clears one whole failed attempt.
...
takes the first of them. One test called `State::begin_open` — for the folder it *refuses*, not the
one it opens — and the worker thread that call spawns records what it opened. Every `cargo test`
therefore put a `…\Temp\km-package-builder-server-<pid>-…` folder at the top of a real list: **eight
of the twelve entries were scratch folders, the corpus was fourth, and the reopen offered an empty
temp directory rather than the corpus somebody had been working on.**

Two things make that hard to see coming. The write is **two levels below the test** — nothing in it
mentions the recent list — and it is **on a spawned thread**, so it lands after the test has passed.
...
`path()` answers `None` under `cfg(test)`, so a list a test holds has nowhere to save to and every
caller — present and future — is covered by one line. `cfg!` rather than `#[cfg]`, so the production
branch still compiles and lints under test.

**The pattern to copy for a new per-user file is to be handed the directory instead.** `km-remote`
and the machine both take `--data-dir`, which is what a second instance uses and what a test run uses
to stay out of the real one. The recent list is the one file no *flag* can name — the question it
answers is asked *before* a corpus is chosen, so no `--data-dir` of the corpus's would reach it —
which is why it needs a guard of its own.
...
through the real binary, and worktree verification corpora and agent scratch folders were opened the
same way — a folder named on the command line being recorded unconditionally, before any scan.
**The guard was working; the population it guarded was drawn too small.** So `cfg(test)` stays, is
checked first, and needs nobody to remember it, while `KM_PACKAGE_BUILDER_RECENT` covers the runs a
`cfg` cannot see: it names the file this run's list goes in, and set-but-empty means it keeps none.
**An integration test is one of those runs** — `tests/upload.rs` links the crate compiled *without*
`cfg(test)`, so a future one that opens a folder must set the variable itself, once, before any
server thread exists. An integration binary being its own process is what makes that safe; a unit
...
the one to be strict about: a run that wrote into the recent list cost somebody a row, where a run
that writes into `machine-passwords.json` is putting a machine's admin password into a store its
owner believes they control. `cfg(test)` is checked first there too, and `src/passwords.rs` takes the
setting as an argument rather than reading it, so its own tests can ask all three questions without
`set_var` at all.

**…and a scratch directory is the same rule one drawer down.** A test that needs a folder takes one
from `Scratch`, which removes it on `Drop` — including after a panic, which a removal written at the
end of a test body does not. Two shapes had escaped it, and both left directories in `%TEMP%` for
good:
...
- **A fixed name, removed only on the way *in*.** Four `db` tests took
  ...
  after. So the directory always survived the run — and because the name carries no process id, **two
...
next** — and the runtime has as many workers as the box has cores, so a handful is the API, the
remote and the event stream stopping together.

The `ops` module's header said everything in it is synchronous because "the controller is a command
channel and the catalog is SQLite; neither awaits". **That premise is not quite true**: queueing
onto an idle machine, and playing or skipping, all reach a path that opens the package, reads the
entry and parses a song before sending anything.

**The rule to carry forward**: a catalog or controller call reached from an `async fn` goes through
`spawn_blocking` **unless it is demonstrably only an atomic read or a channel send.** The three above
all looked like the second and two of them were the first.

**Three more were found in the same shape on 2026-09-07**, which is the argument for the rule rather
than a footnote to it: `play_file`, `play_upload` and `open_audition` all reach `Machine::play_path`,
so a debug play parses a MIDI file, opens an ffmpeg decoder or reads both halves of an MP3+G pair on
a tokio worker — and `open_audition` sweeps older staging folders, a `read_dir` and a
`remove_dir_all` over trees holding whole videos, before a byte of the upload has arrived. They were
missed because the rule was applied per call site, and these three do not *look* like catalog calls.
...
— rather than to keep finding these one at a time.

### A doc link is not checked, and denying `broken_intra_doc_links` was tried and taken out

**Do not add `[workspace.lints.rustdoc]` back without reading this.** It finds real stale links —
about twenty when it was last run, 2026-09-07, eight of them left behind by one rewrite and the rest
renames nobody chased. Those fixes are worth making by hand. The lint still cannot stay on.
...
- **A feature-gated item vanishes with its feature**, so the check would have to name every feature —
  including `km-remote-core/sweep`, which is off everywhere else — and `--all-features` is out for
  the reason at the top of this file.
- Several links need `()` or a full path to disambiguate a function from a module (`arrange`,
  `known`, `path`), which is a fair thing to ask and was the least of it.

So the lint's price is a bespoke feature list, two crates excluded, and a handful of private items
made `pub(crate)` for documentation's sake. **The drift it catches is real but slow**; a rename that
breaks a link is caught by a reader following it, and this repository has readers. If it is tried
again, the thing to change first is the *policy* — decide that a doc link may only name a public
item — rather than the lint setting.
...
recovered in `server.rs` and panicked on at nine sites in `scan.rs`, so a handler that panicked while
holding it left the web UI working and the next corpus scan dying. That was nobody's decision — it
was two files not knowing about each other.
...
`Sequencer::dispatch` is where a per-song correction is applied: a bank select dropped, a program
substituted, a channel silenced. **`seek_ticks` does not go through it.** It scans the event list
itself, fills last-value-wins tables of controllers and programs, and emits into the sink directly —
so a filter written in one place and not the other is a filter that works until somebody seeks.

The failure is quiet and it is not confined to the seek. A song played straight through sounds
corrected; the first seek re-establishes the bank select the fix removed, and the *rest of the song*
then plays the drum kit the correction existed to prevent. Switching the SoundFont mid-song replays
...
second one is in the replay loops. `a_seek_does_not_restore_a_suppressed_bank_select` is the assertion,
and `a_seek_replays_a_bank_select_that_is_not_suppressed` sits beside it so an absence is known to be
caused rather than vacuous — an absence asserted with nothing producing it passes for ever.
...
- **Two checkouts pointed at one build directory share *artifacts*, not just cargo's lock.** Cargo
  will hand a cached test binary compiled in checkout A to a run in checkout B whenever the sources
  match, with the manifest directory baked in. That surfaced as nine tests failing on a fixture path
  under **another worktree's** directory — one that by then did not exist. **It reads exactly like
  somebody else broke master.**
- **The per-worktree slot could not work for the case it was written for.** It was delivered by
  rewriting a settings file whose environment block is resolved **when a session starts** — and
  entering a worktree relocates a session that is already running, so it keeps the parent's value and
  the fresh slot is never written to. Measured: two slots created and never used, while a third
  collected builds run from inside them.
- **Nothing inside a checkout can repair that.** A real `CARGO_TARGET_DIR` beats every configuration
  ...
  `force = true`, because the variable is read by *cargo itself* before that table is applied. **This
  is the second cargo-config assumption this project has had to disprove by experiment** — the first
...
- **Every content file is from a stranger, and three of the things that read one are not this
  project's code.** The trust boundary is the file, not the network: a package, a wallpaper pack, a
  SoundFont bank, a video and a `.kmbuild` are all traded between people who have never met, and the
  machine takes a package the operating system hands it **before anybody has typed a password**. What
  follows from that is written down where it is enforced — `is_safe_path` and `is_safe_name` in
  ...
  `unsafe_code` is denied workspace-wide and the one allowance is console detection, so nothing that
  parses a file is unsafe, and the failures worth hunting are traversal and exhaustion rather than
  corruption. **The C libraries are the other half and are not defensible from here.** A hostile
  video reaches libavformat and libavcodec, which is a large C codebase with a steady supply of
  advisories and no mitigation this crate can apply; the answer is the ffmpeg version, not the code
  around it. **There is no fuzzing**, and the two cheapest targets — `km_song::Song::parse` and
  `km_cdg::GraphicsStream::stats`, both of which take `&[u8]` directly — are five lines each if
  somebody wants the coverage.
- **`rustysynth` is a git dependency on a fork, so it sits outside advisory coverage**, and a
  `.sf2` is a file a stranger sends. What holds it still is `Cargo.lock` plus `--locked` on every
  release path rather than a `rev` in the manifest — the reasoning for that is beside the dependency
  itself. What is worth knowing here is the part an advisory feed would otherwise have told you:
  **the fork's SF2 reader is visibly more careful than upstream's** — `try_reserve_exact` rather than
...
- **The wallpaper tool's cache holds thousands of stock originals, and some providers' terms forbid
  ...
  that**: it is a local cache, gitignored, never distributed, and deleting it would not make a past
  download un-happen. It is listed rather than tasked because it is a position taken knowingly, and a
  later reader who finds the clause should not have to work out whether anybody had noticed. What *is*
  acted on is distribution.
...
- **The curation database's `songs` table is rewritten once**, and it is the only migration in that
  tool that is not an `ADD COLUMN` — **the one place a mistake would destroy a corpus's worth of hand
  curation rather than merely fail.** Two tests cover it and the column list is derived rather than
  written down. Worth a backup before the first open of a large corpus all the same.
...
- **Melody-detection false positives are worse than no detection** — a wrongly muted channel ruins a
  song. The confidence gate, and the test that asserts *abstention* on ambiguous files, are the guard.
...
**Manual checks that matter and cannot be automated**: the syllable highlight lands on the beat at
1.0×, 0.75× and 1.25× tempo; transpose ±6 leaves drums unchanged; the melody toggle affects only the
declared channel and leaves no stuck notes; skip mid-song silences all sounding notes; the queue
advances automatically on song end; wallpapers crossfade without stutter and lyrics stay readable over
bright images; the connect panel shows a URL that actually works from a phone, **and a truthful
message instead** when bound to loopback, offline, or when the server failed to start; and a phone on
the same LAN finds the machine by QR, by typing the shown URL, and by mDNS.

**Two more that need two devices**, and they are the offline remote's: a favorites folder shown on
one phone's screen and read by another's camera lands in the folder the reader was standing in and
adds only — the code is never on screen while it is being read, and a code from a differently named
folder says so before anything is written; and a backup saved from one device restores onto a second
with nothing removed, twice in a row adding nothing the second time. On Android the camera cases only
a device shows are worth walking: granted, denied, **denied twice** (which stops the prompt for good,
so the page must say the camera was refused and name Settings), and the prompt dismissed by a tap
outside. Each must end in a sentence rather than in "Starting the camera…".
