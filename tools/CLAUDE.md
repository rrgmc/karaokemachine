...
builds the native shells, `platform/` is what Linux, macOS and Windows each ask for, `dist/` turns a
build into something you hand over, `dev/` is the working session, and `setup/` is what a build here
needs.
...
`i18n/` beside its own templates. **A new page string is a key**, and the parity tests in that
program's `words.rs` fail the build if the two locales disagree, if the markup asks for something no
catalog has, or if a template prints a word of its own. `--help` in either program is a doc comment
in English.
