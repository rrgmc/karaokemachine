//! The windowed executable: `km-admin`.
//!
//! Three lines of code and one attribute, and the attribute is the whole reason this file is
//! separate from `src/bin/km-admin-console.rs`. `#![windows_subsystem]` applies to a *binary crate
...
//! what pays it is the console twin beside it.
//!
//! The condition is narrow on purpose. Without the `desktop` feature there is no window, so this is
//! the browser-served tool and must keep talking to whoever started it. And macOS is left out
//! because it has no subsystem to choose: a bundled `.app` already has no terminal, a bare
//! executable run from Terminal still prints, and there is no flash on either path.
#![cfg_attr(all(windows, feature = "desktop"), windows_subsystem = "windows")]

fn main() -> anyhow::Result<()> {
    km_admin::run(km_admin::Shell::Windowed)
}
